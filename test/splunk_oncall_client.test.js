...
const SplunkOnCallClient = require('../lib');
...
const clientRewire = rewire('../lib/splunk_oncall_client');
...
const apiIdHeaderName = clientRewire.__get__('API_ID_HEADER_NAME');
const apiKeyHeaderName = clientRewire.__get__('API_KEY_HEADER_NAME');

describe('Splunk On-Call Client Tests', () => {
  let client;
  ...
  beforeEach(() => {
    client = new SplunkOnCallClient(clientOptions);
  });
  
  context(`'SplunkOnCallClient' Class instance`, () => {
    it(`should be an instance of 'SplunkOnCallClient'`, () => {
      expect(client).to.be.an('object');
      expect(client).to.be.an.instanceof(SplunkOnCallClient);
    });
  ...
  context('#constructor()', () => {
    it(`should have a private '_options' property`, () => {
      ...
      expect(() => {
        otherClient = new SplunkOnCallClient();
      }).to.throw();
  ...
  context('#_checkOptions()', () => {
    ...
    it(`should throw an error when missing an 'apiId'`, () => {
      ...
      expect(() => client._checkOptions(options)).to.throw(Error,
        'A Splunk On-Call Api ID is a required option.');
    });
    ...
    it(`should throw an error when missing an 'apiKey'`, () => {
      ...
      expect(() => client._checkOptions(options)).to.throw(Error,
        'A Splunk On-Call Api Key is a required option.');
    });
  ...
  context('#_getRequestHeaders()', () => {
    it('should get the request headers', () => {
      const expectedHeaders = {
        ...
        [apiIdHeaderName]: clientOptions.apiId,
        [apiKeyHeaderName]: clientOptions.apiKey,
  ...
  context('#_performRequest()', () => {
    ...
    it('should return a Splunk On-Call API response', async () => {
    ...
    it('should return a full Splunk On-Call API response', async () => {
      // Mock the API request.
      nock(baseUrl, { reqheaders: client._headers })
        .get('/api-public/v1/user')
        .reply(200, response);
